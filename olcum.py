...
for model in ollama.list()['models']:
    print(model['model'])
    mevcut_modeller.append(model['model'])

# Kullanıcıdan model ismini alıyoruz
secilen_model_ismi = ''
...
def modeli_test_et(model_ismi):
    model = ollama.list()['models'][mevcut_modeller.index(model_ismi)]    
    model_detayli_sonuc = {
        'model': model['model'],
    ...
    model_bolum_sonuc = {'model': model['model']}
    ...
    for i in range(len(mmlu_veri)):
        ...
        response = generate(model=model_ismi, prompt=prompt, think=False, options=Options(seed=42, num_predict=42))
        cevap = response['response']
        # Cevabı veriye ekleme
        mmlu_veri.loc[i, model['model'] + '_cevap'] = cevap
...
modeli_test_et(secilen_model_ismi)
